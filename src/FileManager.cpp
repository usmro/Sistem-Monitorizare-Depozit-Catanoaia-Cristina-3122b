...
Produs* FileManager::produsFromCSV(const std::string& line) {
    ...
    try {
        ...
        if (tip == "perisabil" && f.size() >= 9)
            return new ProdusPerisabil(id, nume, cant, pret, prag, loc, f[7], std::stod(f[8]));
        if (tip == "electronic" && f.size() >= 9)
            return new ProdusElectronic(id, nume, cant, pret, prag, loc, f[7], std::stoi(f[8]));
        if (tip == "isbn" && f.size() >= 11)
            return new ProdusISBN(id, nume, cant, pret, prag, loc, f[7], f[8], f[9], std::stoi(f[10]));
...
void FileManager::incarcaStoc(Depozit& d, const std::string& cale) {
    std::ifstream f(cale);
    if (!f) return; // fisier absent la primul start e normal
    std::string line;
    int ok = 0, erori = 0;
...
void FileManager::incarcaSoferiVehicule(Depozit& d,
                                         ...
                                         const std::string& caleVehicule) {
    {
        ...
        while (std::getline(f, line)) {
            ...
            try {
                d.adaugaSofer(new Sofer(std::stoi(fields[0]), fields[1], fields[2], fields[3]));
            } catch (...) {}
        }
    }
    {
        ...
        while (std::getline(f, line)) {
            ...
            if (fields.size() < 4) continue;
            try {
                d.adaugaVehicul(new Vehicul(std::stoi(fields[0]), fields[1],
                                             std::stod(fields[2]), fields[3]));
            } catch (...) {}
        }
    }
}
...
void FileManager::exportStoc(const Depozit& d, const std::string& cale) {
    ...
    f << "tip,id,nume,cantitate,pret,pragAlerta,locatie,detalii\n";
    for (auto& kv : d.getProduse())
        f << kv.second->toCSV() << "\n";
...
void FileManager::exportIstoric(const Depozit& d, const std::string& cale) {
    std::ofstream f(cale);
    if (!f) { std::cerr << "Nu pot crea: " << cale << "\n"; return; }
    f << "tip,sofer_id,sofer,vehicul_id,vehicul_nr,produs_id,produs,cantitate,timestamp\n";
